...
class PhotoSurface:
    def __init__(self, root):
        self.photo_frame = Frame(root, width=app["width"], height=app["height"])
        root.bind("<Configure>", lambda event: self.photo_frame_auto_resize(event, root), add="+")
        self.lb = Label(self.photo_frame, text='请拍下一张照片......', bd=0, bg="#171841",
                        fg="white")  # todo 要从任务库抽取任务
        self.upload_button = Label(self.photo_frame, text='upload', bg="#171841", fg="white", cursor='hand2')
        self.upload_button.bind('<Button-1>', self.upload_photo)
        self.exit_button = Label(self.photo_frame, text='exit', bg="#171841", fg="white", cursor='hand2')
